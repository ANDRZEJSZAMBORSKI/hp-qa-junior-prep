...
def test_mock_transport_get_and_post():
...
def test_mock_transport_method(method, status, body):
    captured_request = None
    def handler(request: httpx.Request) -> httpx.Response:
        nonlocal captured_request
        captured_request = request
...
def test_mock_transport_path_2(path, code, jsn):
    def handler(request: httpx.Request) -> httpx.Response:
        resp = {
                "/users/1": (200, {"id": 1,"name": "John",}),
                "/users/2": (404, {"error": "User not found",}),
...
@pytest.mark.mocks
@pytest.mark.mocks_httpx
def test_mock_transport_connect_error():
    def handler(request: httpx.Request) -> httpx.Response:
...
@pytest.mark.mocks
@pytest.mark.mocks_httpx
def test_mock_transport_read_timeout():
    def handler(request: httpx.Request) -> httpx.Response:
...
def test_mock_transport_read_timeout_connect_error(error, msg):
    def handler(request: httpx.Request) -> httpx.Response:
        raise error(
            msg,
            request=request,
...
@pytest.mark.mocks
@pytest.mark.mocks_httpx
def test_mock_transport_logs_request(caplog):
    def handler(request: httpx.Request) -> httpx.Response:
        raise httpx.ConnectError(
            "connection failed",
            request=request,
...
@pytest.mark.mocks
@pytest.mark.mocks_httpx
def test_mock_transport_with_retry():
    attempts = 0
...
@pytest.mark.mocks
@pytest.mark.mocks_httpx
def test_mock_transport_with_my_retry():
    attempts = 0
...
@pytest.mark.mocks
@pytest.mark.mocks_httpx
def test_mock_transport_with_retry_exhausted():
    attempts = 0
